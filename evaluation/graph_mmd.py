...
class GEDEvaluator:
    ...
    def evaluate(self, original_graphs, reconstructed_graphs):
        ged_values = []
        for G_orig, G_recon in zip(original_graphs, reconstructed_graphs):
            ged_values.append(self._compute_ged(G_orig, G_recon))

        max_ged = max(ged_values) if ged_values else None
        return {"max_ged": max_ged, "ged_values": ged_values}
